...
class MeanReversionStrategy(Strategy):
    ...
    async def on_bar(
        self, context: StrategyContext, candles: List[Candle]
    ) -> List[StrategySignal]:
    ...
    def calculate_quantity(
        self, signal: StrategySignal, account, current_position_qty: float = 0.0
    ) -> float:
        """
        ...
        if signal.type == SignalType.SELL and current_position_qty > 0:
            ...
            amount = (
                pow(2, -value_diff / target_value)
                * signal.confidence
                * (target_value / 5)
            )
            qty = min(
                amount // current_price, current_position_qty
            )  # floor division parity

...
class DataService:
    ...
    async def _get_existing_records(self, symbol: str, timeframe: str) -> List[DataDownloadRecord]:
        """Get all download records for a symbol/timeframe combination"""
        stmt = select(DataDownloadRecord).where(
            and_(
    ...
    def _intervals_overlap_or_adjacent(self, s1: date, e1: date, s2: date, e2: date) -> bool:
    ...
    async def _update_download_records(self, symbol: str, timeframe: str, new_start: date, new_end: date):
        """
        ...
        for rec in existing_records:
            ...
            rec_start = rec.start_date.date() if hasattr(rec.start_date, 'date') else rec.start_date
            rec_end = rec.end_date.date() if hasattr(rec.end_date, 'date') else rec.end_date
            
            if self._intervals_overlap_or_adjacent(rec_start, rec_end, new_start, new_end):
        ...
        if overlapping_ids:
            ...
            await self.db.execute(
                delete(DataDownloadRecord).where(DataDownloadRecord.id.in_(overlapping_ids))
            )
    ...
    async def download_historical(
        ...
        log = LogEntry(
            ...
            source="DataService",
            message=f"Downloaded {saved_count} candles for {len(symbols)} symbols ({timeframe})",
            context={"symbols": symbols, "timeframe": timeframe, "start": str(start_date), "end": str(end_date)}
        )
        self.db.add(log)
        await self.db.commit()

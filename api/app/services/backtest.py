...
class BacktestService:
    ...
    async def _ensure_backtest_data_ready(
        ...
        if still_missing:
            if settings.BROKER_MODE.lower() == "kis" and timeframe.lower() in {
                "1min",
                "1m",
                "5min",
                "5m",
    ...
    async def run_backtest(
        ...
        lookback = getattr(strategy, "params", {}).get("duration", 20) + getattr(
            strategy, "params", {}
        ).get("candle_buffer", 10)
        ...
        try:
            ...
            if not candle_map:
                raise ValueError(
                    f"No data available for {symbols} ({strategy.timeframe}) from {start_date} to {end_date}"
                )
            ...
            for time_idx, current_time in enumerate(sorted_times):
                ...
                for sym, current_candle in time_candles.items():
                    ...
                    context = StrategyContext(
                        ...
                        params={
                            "start_point_ts": start_point_ts.get(sym),
                            **(params or {}),
                        },
                    )
                    ...
                    for sig in signals:
                        sig.metadata.setdefault(
                            "bar_timestamp", current_candle.timestamp
                        )
                        sig.metadata.setdefault(
                            "prev_bar_timestamp", current_slice[-2].timestamp
                        )
                        signals_log.append(
                            {
                                "symbol": sym,
                    ...
                    for sig in signals:
                        ...
                        qty = strategy.calculate_quantity(
                            sig, mock_account, current_pos_qty
                        )
                        ...
                        if sig.type == SignalType.BUY and qty > 0:
                            ...
                            if cash >= cost:
                                ...
                                start_point_ts[sym] = sig.metadata.get(
                                    "prev_bar_timestamp", current_candle.timestamp
                                )
                                trades.append(
                                    {
                                        "symbol": sym,
                ...
                if (
                    time_idx % 5 == 0 or time_idx == total_times - 1
                ):  # Every 5 bars or last

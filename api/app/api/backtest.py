...
async def run_backtest(
    ...
    target_value = (
        resolved_params.get("target_value")
    ...
    if target_value is not None:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid target_value in backtest params"
            )
...
async def get_result(run_id: str, db: AsyncSession = Depends(get_db)):
    ...
    res_res = await db.execute(
        select(BacktestResult).where(BacktestResult.run_id == uuid.UUID(run_id))
    )

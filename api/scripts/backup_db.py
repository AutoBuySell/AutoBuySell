...
def create_backup(output_dir: str) -> str:
    ...
    try:
        ...
        if in_docker and db_config["host"] == "db":
            ...
            print(
                f"  docker exec autobuysell_db pg_dump -U {db_config['user']} {db_config['dbname']} | gzip > {filename}"
            )
            print(
            ...
            with open(output_path, "wb") as f:
                pg_dump = subprocess.Popen(
                    cmd, stdout=subprocess.PIPE, env=env, stderr=subprocess.PIPE
                )
                ...
                if pg_dump.returncode != 0:
                    error_msg = (
                        pg_dump_stderr.decode() if pg_dump_stderr else "Unknown error"
                    )

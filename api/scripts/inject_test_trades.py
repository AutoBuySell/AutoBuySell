...
async def inject_test_data(symbol: str, num_trades: int, with_commission: bool):
    ...
    async with AsyncSessionLocal() as db:
        ...
        for i in range(num_trades):
            ...
            print(
                f"   [{i + 1:2}] {trade_date.strftime('%Y-%m-%d')} {side.upper():4} {qty:3} @ ${price:.2f} (fee: ${commission:.2f})"
            )
...
async def main():
    ...
    parser.add_argument(
        "--symbol", type=str, default="AAPL", help="Symbol to inject data for"
    )
    parser.add_argument(
        "--trades", type=int, default=10, help="Number of trades to generate"
    )
    parser.add_argument(
        "--with-commission", action="store_true", help="Include non-zero commission"
    )

        ...
        for strategy in strategies:
            ...
            if params:
                ...
                for param in params:
                    ...
                    print(
                        f"   {f'v{param.version}':<10} {symbol_display:<15} {active:<8} {params_preview}"
                    )
            else:
...
async def add_strategy(name: str, class_path: str, description: str = None):
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(select(StrategyMeta).where(StrategyMeta.name == name))
...
async def update_strategy_params(
    name: str, params_json: str, symbol: str = None, make_active: bool = False
):
    ...
    async with AsyncSessionLocal() as db:
        ...
        existing_result = await db.execute(
            ...
            .where(
                ...
                StrategyParam.symbol == symbol
                if symbol
                else StrategyParam.symbol.is_(None),
            )
            .order_by(StrategyParam.version.desc())
        )
        ...
        print(
            f"✅ Parameters added for '{name}' (Symbol: {symbol_display}, v{next_version})"
        )
        if make_active:
...
async def delete_strategy(name: str, force: bool = False):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not force:
            print(
                f"⚠️  This will delete strategy '{name}' and {params_count} parameter version(s)!"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage AutoBuySell Trading Strategies"
    )
    ...
    add_parser.add_argument(
        "class_path", help="Python class path (e.g., app.strategies.sma.SMAStrategy)"
    )
    ...
    update_parser.add_argument(
        "--symbol", help="Specific symbol (optional, defaults to global)"
    )
    update_parser.add_argument(
        "--active", action="store_true", help="Set as active parameters"
    )
    ...
    if args.command == "list":
        ...
        asyncio.run(
            update_strategy_params(args.name, args.params, args.symbol, args.active)
        )
